...
<div class="container-fluid">
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="filterOffcanvas" aria-labelledby="filterOffcanvasLabel">
        ...
        <div class="offcanvas-body">
            <form id="filter-form" action="{{ route('reseller.agent') }}" method="GET">
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <select name="country" class="form-select">
                        ...
                        @foreach(\App\Models\User::where('account_type', 'reseller_agent')->whereNotNull('country')->where('country', '!=', '')->distinct()->orderBy('country')->pluck('country') as $name)
                ...
                <div class="mb-4">
                    ...
                    <select name="state" class="form-select">
                        ...
                        @foreach(\App\Models\User::where('account_type', 'reseller_agent')->whereNotNull('state')->where('state', '!=', '')->distinct()->orderBy('state')->pluck('state') as $name)
                ...
                <div class="d-grid gap-2 pt-3 border-top">
                    ...
                    <a href="{{ route('reseller.agent') }}" class="btn btn-light">Reset All</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            <div>
                <h5 class="mb-0 fw-bold">Reseller Agent Management</h5>
                <small class="text-muted total-count">{{ $users->total() }} Resellers Found</small>
            </div>
            <div class="d-flex gap-2">
                <a href="{{ route('reseller.agent.export', request()->all()) }}" id="csv-export-link" class="btn btn-success btn-sm px-3 shadow-sm">
                    <i class="fas fa-file-csv me-1"></i> CSV
                </a>
                @if(auth()->user()->account_type !== 'manager' || auth()->user()->can_create_user)
                <a href="{{ route('users.create') }}?role=reseller_agent" class="btn btn-primary btn-sm px-3 shadow-sm">
        ...
        <div class="card-body" id="table-container">
            @include('dashboard.partials.reseller-table')
        </div>
    </div>
</div>
        ...
        function updateTable(url) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let csvUrl = "{{ route('reseller.agent.export') }}?" + (url.split('?')[1] || '');
